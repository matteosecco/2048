...
class App:
    ...
    def callback(self, direction):
        ...
        if self._canclick:
            ...
            self.root.after(cell.WAIT * 350, self._spawnnew)
        ...
        for idx in list(range(len(self.table)))[::-1]:  # Iterates the array backwards
            ...
            if self.table[idx]:  # Checks if there's a cell
                ...
                while (idx + 1) % 4:  # Keeps going till it reaches the left side
                    ...
                    if isinstance(self.table[idx + 1], cell.Cell) and self.table[idx + 1].n == self.table[idx].n:
        ...
        for idx in range(len(self.table)):  # # Iterates the array from the beginning
            ...
            if self.table[idx]:
                ...
                while idx % 4:
                    ...
                    if isinstance(self.table[idx - 1], cell.Cell) and self.table[idx].n == self.table[idx - 1].n:
        ...
        for idx in list(range(len(self.table)))[::-1]:
            ...
            if self.table[idx]:
                ...
                while not 12 <= idx <= 15:
                    ...
                    if isinstance(self.table[idx + 4], cell.Cell) and self.table[idx + 4].n == self.table[idx].n:
...
root.bind("<d>", lambda event: app.callback(LEFT))
root.bind("<a>", lambda event: app.callback(RIGHT))
root.bind("<w>", lambda event: app.callback(UP))
root.bind("<s>", lambda event: app.callback(DOWN))
